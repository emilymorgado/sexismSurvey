...
// Symbols for Custom Type Formatting:
const ctfSymbols = {
    toPostgres: Symbol('ctf.toPostgres'),
    rawType: Symbol('ctf.rawType')
};
...
function formatValue(value, fm, cc) {
    ...
    switch (typeof value) {
        ...
        case 'symbol':
            throw new TypeError('Type Symbol has no corresponding PostgreSQL type: ' + value.toString());
        default:
...
function getCTF(value) {
    if (!isNull(value)) {
        ...
        if (typeof value.toPostgres === 'function') {
            return {toPostgres: value.toPostgres, rawType: !!value._rawType};
        }
    }
    return null;
}
...
const $as = {
     ...
     *
     * Note that `rawType` does not have a leading underscore in its name, because with the old approach it was added for additional uniqueness of
     * the property, should the properties be enumerated. And since ES6 $[Symbol] is unique by definition, there is no more need for the
     ...
     * Property name for the $[Custom Type Formatting] flag `rawType` (legacy flag `_rawType`).
     *
     * @example
     ...
     * class MyType {
     *     constructor() {
     *         this[ctf.rawType] = true; // set it only when toPostgres returns a formatted result
     *     }
     *
     *     [ctf.toPostgres](self) {
     *         // self = this
     ...
     * - **Version 6.10.0** added support for $[Nested Named Parameters].
     *
